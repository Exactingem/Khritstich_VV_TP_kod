...
class DataDrawApp:
    def __init__(self, root):
        ...
        self.selected_y = tk.StringVar(value=self.all_columns[1] if len(self.all_columns) > 1 else self.all_columns[0] if self.all_columns else "")
        
        self.color_scheme = tk.StringVar(value="BuGn")
        self.color_schemes = ['BuGn', 'Blues', 'Greens', 'Oranges', 'Reds', 'Purples', 'Greys', 'viridis', 'plasma', 'inferno', 'magma', 'cividis']
        
        # ID студента: 70227954
        student_id = "70227954"
        r = int(student_id[-6:-4])  # 22
        g = int(student_id[-4:-2])  # 79
        b = int(student_id[-2:])    # 54
        ...
        self.lines_history = []  # Список линий [(xs, ys, color, width), ...]
        self.current_line_x = []
        self.current_line_y = []
        self.temp_line = None
    ...
    def setup_ui(self):
        ...
        self.width_spinbox = ttk.Spinbox(width_frame, from_=1, to=50, width=8, textvariable=tk.IntVar(value=self.pen_width))
        ...
        self.x_combo = ttk.Combobox(bottom_frame, textvariable=self.selected_x, values=self.all_columns, state="readonly", width=20)
        ...
        self.y_combo = ttk.Combobox(bottom_frame, textvariable=self.selected_y, values=self.all_columns, state="readonly", width=20)
        ...
        self.color_combo = ttk.Combobox(bottom_frame, textvariable=self.color_scheme, values=self.color_schemes, state="readonly", width=10)
    ...
    def on_column_change(self):
        if self.drawing_mode:
            ...
            self.draw_button.state(['!pressed'])
            self.canvas.get_tk_widget().config(cursor="")
        self.lines_history = []  # Очищаем историю при смене данных
        self.update_plot()
    ...
    def on_motion(self, event):
        ...
        self.temp_line = self.ax.plot(self.current_line_x, self.current_line_y,
    ...
    def on_release(self, event):
        ...
        if event.button == 1:
            ...
            # Сохраняем линию, если есть что сохранять
            if len(self.current_line_x) > 1:
                # Рисуем постоянную линию
                line = self.ax.plot(self.current_line_x, self.current_line_y,
                                   color=self.pen_color, linewidth=self.pen_width,
                                   solid_capstyle='round', solid_joinstyle='round')[0]
                # Сохраняем линию в историю
                self.lines_history.append({
                    'x': self.current_line_x.copy(),
                    'y': self.current_line_y.copy(),
    ...
    def undo_last_line(self, event=None):
        ...
        if not self.lines_history:
            return
        # Удаляем последнюю линию из истории
        self.lines_history.pop()
        self.redraw_with_lines()
    
    def redraw_with_lines(self):
        """Перерисовывает график со всеми линиями из истории"""
        self.update_plot()
        # Восстанавливаем все линии из истории
        for line_data in self.lines_history:
            self.ax.plot(line_data['x'], line_data['y'],
                        color=line_data['color'], linewidth=line_data['width'],
    ...
    def plot_scatter(self, x_col, y_col):
        ...
        self.ax.scatter(x_data[common_idx], y_data[common_idx], marker='<', color=color, alpha=0.6, s=80, edgecolors='darkblue', linewidth=0.5)
